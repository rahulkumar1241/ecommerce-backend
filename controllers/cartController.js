...
const handleAddToCart = async (req, res) => {
    ...
    if (error) {
        ...
        try {
            ...
            let query = `SELECT * FROM ecommercedb.cart where product_id='${product_id}' AND user_id='${decoded.user_id}'`;
            ...
            if (result.length) {
                ...
                const [resultAddTocart] = await pool.execute(
                    'INSERT INTO ecommercedb.cart (product_id, user_id,quantity,datetime) VALUES (?, ?, ?, ?)',
...
const handleChangeQuantity = async (req, res) => {
    ...
    if (error) {
        ...
        try {
            ...
            const decoded = jwt.verify(token, process.env.SECRET_KEY);
            if (quantity === 0) {
                let query = `DELETE FROM ecommercedb.cart where product_id='${product_id}' AND user_id='${decoded.user_id}'`;
                const [result] = await pool.execute(query);
                res.status(200).json({
                ...
                let query = `UPDATE ecommercedb.cart SET quantity = '${quantity}' WHERE user_id='${decoded.user_id}' AND product_id='${product_id}'`;
                const [result] = await pool.execute(query);
...
const handleUpdateUserInfo = async (req, res) => {
    ...
    try {
        ...
        // Verify the token and decode its payload
        const decoded = jwt.verify(token, process.env.SECRET_KEY);
        let query = `UPDATE ecommercedb.users SET firstname = '${firstname}',lastname = '${lastname}',photo_url='${photo_url}' WHERE user_id = '${decoded.user_id}'`;
        const [result] = await pool.execute(query);
        res.status(200).json({
            status: 200,
