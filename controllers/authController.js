...
const handleSignupController = async (req, res) => {
    ...
    if (error) {
        ...
        try {
            const [rows, fields] = await pool.execute(`SELECT * FROM ecommercedb.users where email='${email}'`);
            ...
            if ((rows.length) && rows[0].is_active) {
                ...
                const [result] = await pool.execute(
                    'UPDATE ecommercedb.users SET hash=? WHERE user_id=?',
                    [hash, rows[0].user_id]
                );
                ...
                const [result] = await pool.execute(
                    'INSERT INTO ecommercedb.users (firstname, lastname, email, hash) VALUES (?, ?, ?, ?)',
...
const handleSetPassword = async (req, res) => {
    ...
    if (error) {
        ...
        try {
            const [rows, fields] = await pool.execute(`SELECT * FROM ecommercedb.users where hash='${hash}'`);
            ...
            if (rows.length) {
                ...
                const [result] = await pool.execute(
                    `UPDATE ecommercedb.users SET password= ?, is_active= ?  WHERE hash= ?`,
        ...
        try {
            ...
            if (rows.length) {
                if (!rows[0].is_active) {
                    ...
                    const response = await sendEmail(data);
                    const [result] = await pool.execute(
                        'UPDATE ecommercedb.users SET otp =? WHERE email=?',
                        [otp, email]
                    );
                    res.status(200).json({
                        success: true,
    ...
    if (error) {
        ...
        try {
            ...
            if (rows.length) {
                ...
                const [result] = await pool.execute(`UPDATE ecommercedb.users SET otp = '${otp}' WHERE user_id='${decoded.user_id}'`);
...
const handleVerifyOtpUpdateEmail = async (req, res) => {
    ...
    try {
        ...
        const [result, fields] = await pool.execute(`SELECT * FROM ecommercedb.users where user_id='${decoded.user_id}'`);

        if (result[0].otp === otp) {
            ///////////////update the email now///////////////////
            const [result] = await pool.execute(`UPDATE ecommercedb.users SET email= '${email}' WHERE user_id='${decoded.user_id}'`);
