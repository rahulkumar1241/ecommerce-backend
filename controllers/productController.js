...
const handleGetProduct = async (req, res) => {
    ...
    if (error) {
        ...
        try {
            const [rows, fields] = await pool.execute(`SELECT * FROM ecommercedb.products where product_id='${product_id}'`);
            if (rows.length) {
                let token = req.headers.authorization;
                // Verify the token and decode its payload
                const decoded = jwt.verify(token, process.env.SECRET_KEY);

                const [recentlyViewedData] = await pool.execute(`SELECT * FROM ecommercedb.recently_viewed_products where product_id='${product_id}' AND user_id='${decoded.user_id}'`);
                ...
                if (recentlyViewedData.length) {
                    ...
                    const [result] = await pool.execute(
                        `UPDATE ecommercedb.recently_viewed_products SET watch_datetime='${datetime}' WHERE product_id='${product_id}' AND user_id='${decoded.user_id}'`,
                    );
                    ...
                    const [result] = await pool.execute(
                        'INSERT INTO ecommercedb.recently_viewed_products (product_id, user_id,watch_datetime) VALUES (?, ?, ?)',
...
const handleAddToWishlist = async (req, res) => {
    ...
    if (error) {
        ...
        try {
            ...
            const [rows, fields] = await pool.execute(`SELECT * FROM ecommercedb.wishlist where product_id='${product_id}' AND user_id='${decoded.user_id}'`);
            ...
            if (rows.length) {
                ...
                ///////////INSERT///////
                const [result] = await pool.execute(
                    'INSERT INTO ecommercedb.wishlist (product_id, user_id,datetime) VALUES (?, ?, ?)',
...
const getWishListItems = async (req, res) => {
    ...
    try {
        ...
        ecommercedb.wishlist AS wishlist
        LEFT JOIN 
        ecommercedb.products AS products ON 
...
const handleRemoveFromWishList = async (req, res) => {
    ...
    if (error) {
        ...
        const decoded = jwt.verify(token, process.env.SECRET_KEY);
        try {
            const [result] = await pool.execute(`DELETE FROM ecommercedb.wishlist where product_id='${product_id}' AND user_id='${decoded.user_id}'`);
...
const handleGetAllProducts = async (req, res) => {
    ...
    if (error) {
        ...
        try {
            ...
            let query = `SELECT 
            SQL_CALC_FOUND_ROWS
             * FROM ecommercedb.products AS products 
            LEFT JOIN 
            ecommercedb.categories AS categories
...
const handleUpdateAllProducts = async (req, res) => {
    const [rows] = await pool.execute("SELECT * from ecommercedb.products");
    for (let i = 0; i < rows.length; ++i) {
        await pool.execute(`UPDATE ecommercedb.products SET price_after_discount ='${rows[i].price - Math.floor((rows[i].price / 100) * rows[i].discount_per)}' WHERE product_id='${rows[i].product_id}'`,);
    }
    res.status(200).json({
        status: 200,
        success: true,
