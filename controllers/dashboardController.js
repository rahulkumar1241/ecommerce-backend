...
const handleMainPage = async (req, res) => {
    ...
    try {
        ...
        const [rows, fields] = await pool.execute(`SELECT * FROM ecommercedb.categories`);

        for (let i = 0; i < rows.length; ++i) {

            let query = `SELECT SQL_CALC_FOUND_ROWS * FROM ecommercedb.products where category='${rows[i].cat_id}' LIMIT ${LIMIT}`;
        ...
        SELECT * FROM ecommercedb.recently_viewed_products AS recently_viewed_products
        LEFT JOIN 
        ...
        res.status(200).json({
            data: {
                "product_with_categories": resObj,
                "recently_watched_items": results
            },
            status: 200,
...
const handleGetAllDropdownValues = async (req, res) => {
    ...
    try {
        if (product_categories) {
            let [result] = await pool.query("SELECT * FROM ecommercedb.categories");
        ...
        if (order_status_dropdown) {
            ...
            let query = decoded.role !== 2 ?
                "SELECT * FROM ecommercedb.order_status"
                :
                "SELECT * FROM ecommercedb.order_status WHERE status_id IN ('7', '12')";
...
const handleUploadAllPincodes = async (req, res) => {
    try {
        ...
        for (let i = 0; i < jsonData.length; ++i) {
            ///////////INSERT///////
            await pool.execute(
                'INSERT INTO ecommercedb.pincode (pincode,state,district) VALUES (?, ?, ?)',
...
const handleGetPinCodeDetails = async (req, res) => {
    ...
    if (error) {
        ...
        try {
            ...
            let [result] = await pool.execute(`SELECT * FROM ecommercedb.pincode where pincode='${pincode}'`);
