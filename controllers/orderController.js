    ...
    if (error) {
        ...
        try {
            ...
            const [result] = await pool.execute(
                'INSERT INTO ecommercedb.orders (user_id,total_amount, order_status) VALUES (?, ?, ?)',
            ...
            for (let i = 0; i < products.length; ++i) {
                const [orderItemsResult] = await pool.execute(
                    'INSERT INTO ecommercedb.order_items (user_id,order_id,product_id, quantity ,buyout_price,total) VALUES (?, ?, ?, ?, ?, ?)',
...
const handleUpdateOrder = async (req, res) => {
    ...
    if (error) {
        ...
        try {
            ...
            let [result] = await pool.execute(`SELECT * FROM ecommercedb.orders where order_id='${order_id}'`);
            if (result.length) {
    ...
    if (error) {
        ...
        try {
            ...
            if (result.length) {
                ...
                let [order_items] = await pool.execute(`SELECT * FROM ecommercedb.order_items AS order_items
                LEFT JOIN 
    ...
    if (error) {
        ...
        try {
            ...
            if (result.length) {
                ...
                let [items] = await pool.execute(`SELECT * FROM ecommercedb.order_items where order_id=${order_id}`);

                for (let i = 0; i < items.length; ++i) {

                    await pool.execute(`UPDATE ecommercedb.order_items SET item_status='${orderStatus}',order_item_datetime='${moment(new Date()).format('YYYY-MM-DD HH:mm:ss')}' WHERE order_item_id='${items[i].order_item_id}'`);

                    let [productData] = await pool.execute(`SELECT * FROM ecommercedb.products where product_id='${items[i].product_id}'`);
                    ...
                    await pool.execute(`DELETE FROM ecommercedb.cart where product_id='${items[i].product_id}' AND user_id='${decoded.user_id}'`);

                    await pool.execute(`DELETE FROM ecommercedb.wishlist where product_id='${items[i].product_id}' AND user_id='${decoded.user_id}'`);
...
const handleSendOtpOrder = async (req, res) => {
    ...
    let { order_id, mobile_number } = req.body;
    ...
    if (error) {
        ...
        try {
            ...
            let order_status = 10;
            const [result] = await pool.execute(
                `UPDATE ecommercedb.orders SET otp='${otp}', mobile_number='${mobile_number}',order_status='${order_status}' WHERE order_id='${order_id}'`,
            );
            res.status(200).json({
                success: true,
                status: 200,
            ...
            if (result.length) {
                ...
                if (otp === orgOtp) {
                    ...
                    const [UpdateResult] = await pool.execute(
                        `UPDATE ecommercedb.orders SET is_verified='1',order_status='${order_status}' WHERE order_id='${order_id}'`,
                    );
                    res.status(200).json({
                        status: 200,
                        success: true,
...
const handleGetMyOrders = async (req, res) => {
    ...
    try {
        ...
        ON
        order_items.product_id = products.product_id
        LEFT JOIN ecommercedb.orders AS orders
        ON
...
const handleUpdateOrderStatus = async (req, res) => {
    ...
    if (error) {
        ...
        try {
            ...
            let [result] = await pool.execute(`SELECT * from ecommercedb.order_items AS order_items LEFT JOIN ecommercedb.products AS products on order_items.product_id = products.product_id WHERE order_item_id='${item_id}'`);

            if (status === 13 || status === 12) {
                let query = `UPDATE ecommercedb.order_items SET item_status = '${status}',order_item_datetime = '${moment(new Date()).format('YYYY-MM-DD HH:mm:ss')}' WHERE order_item_id='${item_id}'`
...
const handleGetOrderItemDetails = async (req, res) => {
    ...
    if (error) {
        ...
        let findOrderItemQuery = `SELECT * from ecommercedb.order_items WHERE order_item_id='${order_item_id}'`;
        ...
        if (!result.length) {
            ...
            let query =
                `SELECT order_items.*,products.*,orders.*,order_status.*,users.firstname,users.lastname,order_address_type.* from ecommercedb.order_items as order_items
        LEFT JOIN ecommercedb.products as products
        ON 
        order_items.product_id = products.product_id
        LEFT JOIN  
